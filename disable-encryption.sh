...
# example:
## ./disable-encryption.sh 10.10.10.10 ilo4
## ./disable-encryption.sh 10.10.10.20 ilo5
...
JQ="$HOME/ilo/jq"
json_pkg_4='{ "Oem": { "Hp": { "EnforceAES3DESEncryption": false } } }'
json_pkg_5='{ "SecurityState": "Production" }'
...
if [[ "${encryption_status}" == "Production" ]] || [[ "${encryption_status}" == "false" ]] ; then
        printf "${ilo_host},${host_ilo_version},encryption_already_disabled,${encryption_status}\n"
        exit 0
fi
